...
| D11 | **The render protocol is platform-portable.** A 14KB single-file HTML page renders the same frame ops the M5 firmware does (`clr`/`bnr`/`txt`/`rect`/`fill`/`bmp`/`spr`/`led`/`buz`/`seq`), polls the same `/list` and `/run` endpoints with the same bearer auth, and shares state with the wrist device live. The M5 is not the canonical client — it's the lowest-power proof of a hardware-agnostic protocol. New surfaces (pi TUI, Apple Watch, mobile widgets, etc.) reduce to "implement the same op vocabulary." Total over-the-wire payload: 5.5KB gzipped. | [exp-05b](../experiments/exp-05b-can-browser-render-desk-frames/RESULT.md) | 2026-05-01 |
| D12 | **Bearer in URL hash is sufficient for v0 viewer auth.** The browser viewer at `GET /viewer` is a public, no-auth HTML response. The user's bearer token rides in the URL fragment (`#token=…`), which never reaches the server, is captured by the page on load, and persists in `sessionStorage`. OAuth on `/mcp` and `/viewer` is exp-12-territory. | [exp-05b](../experiments/exp-05b-can-browser-render-desk-frames/RESULT.md) | 2026-05-01 |
...
| F-14 | exp-05b | Rapid B-cycle on browser dock can swallow presses while the first `/list` is still in-flight (dockApps.length=0 clamps `dockIdx` to 0). | ⚠️ open — viewer-side |
| F-15 | exp-05b | Browser viewer doesn't see out-of-band state changes between polls (M5 increments counter while viewer is on it; viewer doesn't update until next press). Active-app polling or WS would fix. | ⚠️ open — minor UX |
| F-16 | exp-05b | "<10KB JS" target couldn't distinguish code from inlined data (font is ~2KB of the JS payload). Future criteria should target gzipped wire size or carve out asset blobs. | 📝 lessons-learned |
| F-17 | exp-05b | On-canvas tap-zones (bottom 1/3 = A, sides = B) overlap with normal mouse interaction patterns. Acceptable for v0 but mobile/touch UX needs explicit tap+held gestures. | ⚠️ open — UX |
