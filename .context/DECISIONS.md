...
| D11 | **The render protocol is platform-portable.** A 14KB single-file HTML page renders the same frame ops the M5 firmware does (`clr`/`bnr`/`txt`/`rect`/`fill`/`bmp`/`spr`/`led`/`buz`/`seq`), polls the same `/list` and `/run` endpoints with the same bearer auth, and shares state with the device live. The M5 is not the canonical client — it's the lowest-power proof of a hardware-agnostic protocol. New surfaces (pi TUI, Apple Watch, mobile widgets, etc.) reduce to "implement the same op vocabulary." Total over-the-wire payload: 5.5KB gzipped. | [exp-05b](../experiments/exp-05b-can-browser-render-desk-frames/RESULT.md) | 2026-05-01 |
